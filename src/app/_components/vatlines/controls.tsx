...
export default function Controls(props: ControlsProps) {
  ...
  const volumeUp = () => {
    props.setVolume(props.volume + 1);
  };
  ...
  return (
    <div className="h-min flex mt-1 text-white">
      <div
        className="w-12 cursor-pointer flex-none my-auto"
        onClick={volumeDown}
      >
        Vol -
      </div>
      <div className="w-min mx-4 outline outline-1 px-1 my-auto">
        {props.volume}%
      </div>
      <div className="w-12 cursor-pointer flex-none my-auto" onClick={volumeUp}>
        Vol +
      </div>
      <div className="flex-grow">
        <p className="text-center">
          {props.metadata.facilityId}-{props.metadata.sector}
          {' ('}
      ...
      <div className="cursor-pointer my-auto" onClick={() => openSettings()}>
        <svg
          viewBox="0 0 52 52"
          xmlns="http://www.w3.org/2000/svg"
          className="fill-white w-5 h-6"
        >
